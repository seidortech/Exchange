...
$logfile = "$myDir\SyncMailboxAddAlias.log"
$MailboxesInCsv= Import-csv "$MyDir\SyncMailboxAliasInput.csv" |%{$_.EmailAlias}
...
{
    $mailbox = Get-RemoteMailbox -Identity $Mailbox
    ...
    if ($Commit)
    {
        ...
        Set-RemoteMailbox -Identity $Mailbox.Alias -EmailAddressPolicyEnabled $false -EmailAddresses $addresses      
    }
    else
    {
        Write-LogFile ""
