...
$logfile = "$myDir\365MailboxAddAlias.log"
$MailboxesInCsv= Import-csv "$MyDir\365MailboxAliasInput.csv" |%{$_.EmailAlias}
$uri = "https://ps.outlook.com/Powershell"
$LiveCred=Get-Credential -Credential youradmin@office365tenant.com
...
$PSSession=New-PSSession -ConfigurationName Microsoft.Exchange -ConnectionUri $uri -Credential $LiveCred -Authentication Basic -AllowRedirection
...
Foreach ($Mailbox in $MailboxesInCsv)
{
    ...
    if ($Commit)
    {
        ...
        Set-Mailbox -Identity $Mailbox.Alias -EmailAddressPolicyEnabled $false -EmailAddresses $addresses      
    }
    else
    {
        Write-LogFile ""
